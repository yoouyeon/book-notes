...
- [책에 인용된 createReactiveObject 함수 내부 구현 코드](https://github.com/vuejs/core/blob/main/packages/reactivity/src/reactive.ts#L268) : 어떤 객체를 관찰 가능한 객체로 만드는 함수 : 변경이 발생했을 때 해당 객체를 구독하는 옵저버들에게 알리는 역할은 `baseHandlers` 나 `collectionHandlers` 에서 수행하게 된다.
