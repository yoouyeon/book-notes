...
block-beta
    columns 5
        mainModule("메인 모듈")
        space
        dependencyInjector("의존성 주입자")
        dependencyInjector --> mainModule
        space:2
        space:5
        space:2
        dependency1("의존성 1")
        dependency2("의존성 2")
        dependency3("의존성 3")
        dependencyInjector --> dependency1
        dependencyInjector --> dependency2
        dependencyInjector --> dependency3
```
...
flowchart LR
    ...
    view -->|"유저 이벤트"| presenter
    presenter -->|"UI 갱신"| view
    model -->|"모델 변경"| presenter
    presenter -->|"모델 갱신"| model
